...
export function BestellungenDialog({ open, onClose, onSubmit, defaultValues, gerichteList, enablePhotoScan = true, enablePhotoLocation = true }: BestellungenDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="gewaehlte_gerichte" entity="Gerichte">\n${JSON.stringify(gerichteList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "gewaehlte_gerichte": string | null, // Display name from Gerichte (see <available-records>)\n  "sonderwunsch": string | null, // Sonderwünsche / Anmerkungen\n  "zahlungsart": LookupValue | null, // Zahlungsart (select one key: "bar" | "paypal" | "ueberweisung" | "sonstiges") mapping: bar=Bar, paypal=PayPal, ueberweisung=Überweisung, sonstiges=Sonstiges\n  "bereits_bezahlt": boolean | null, // Bereits bezahlt\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["gewaehlte_gerichte"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const gewaehlte_gerichteName = raw['gewaehlte_gerichte'] as string | null;
        if (gewaehlte_gerichteName) {
          const gewaehlte_gerichteMatch = gerichteList.find(r => matchName(gewaehlte_gerichteName!, [String(r.fields.gericht_name ?? '')]));
          if (gewaehlte_gerichteMatch) merged['gewaehlte_gerichte'] = createRecordUrl(APP_IDS.GERICHTE, gewaehlte_gerichteMatch.record_id);
        }
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="gewaehlte_gerichte">Gewählte Gerichte</Label>
            <Select
              value={extractRecordId(fields.gewaehlte_gerichte) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, gewaehlte_gerichte: v === 'none' ? undefined : createRecordUrl(APP_IDS.GERICHTE, v) }))}
            >
              <SelectTrigger id="gewaehlte_gerichte"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
