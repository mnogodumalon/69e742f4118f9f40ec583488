...
const GERICHTE_FIELDS = [
  ...
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'vorspeise', label: 'Vorspeise' }, { key: 'hauptgericht', label: 'Hauptgericht' }, { key: 'beilage', label: 'Beilage' }, { key: 'dessert', label: 'Dessert' }, { key: 'getraenk', label: 'Getränk' }, { key: 'sonstiges', label: 'Sonstiges' }] },
...
const BESTELLUNGEN_FIELDS = [
  ...
  { key: 'gewaehlte_gerichte', label: 'Gewählte Gerichte', type: 'applookup/select', targetEntity: 'gerichte', targetAppId: 'GERICHTE', displayField: 'gericht_name' },
  { key: 'sonderwunsch', label: 'Sonderwünsche / Anmerkungen', type: 'string/textarea' },
  { key: 'zahlungsart', label: 'Zahlungsart', type: 'lookup/radio', options: [{ key: 'bar', label: 'Bar' }, { key: 'paypal', label: 'PayPal' }, { key: 'ueberweisung', label: 'Überweisung' }, { key: 'sonstiges', label: 'Sonstiges' }] },
...
export default function AdminPage() {
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'gerichte':
        lists.restaurantList = (data as any).restaurant ?? [];
        break;
      case 'bestellungen':
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'gerichte' && fieldKey === 'restaurant_ref') {
      const match = (lists.restaurantList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.restaurant_name ?? '—';
    }
    if (entity === 'bestellungen' && fieldKey === 'gewaehlte_gerichte') {
