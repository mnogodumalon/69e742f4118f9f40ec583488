...
import type { Restaurant, Gerichte, Bestellungen, CreateRestaurant, CreateGerichte, CreateBestellungen } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getRestaurant(): Promise<Restaurant[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.RESTAURANT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Restaurant[];
    return enrichLookupFields(records, 'restaurant');
  }
  static async getRestaurantEntry(id: string): Promise<Restaurant | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.RESTAURANT}/records/${id}`);
    const record = { record_id: data.id, ...data } as Restaurant;
    return enrichLookupFields([record], 'restaurant')[0];
  }
  static async createRestaurantEntry(fields: CreateRestaurant) {
    return callApi('POST', `/apps/${APP_IDS.RESTAURANT}/records`, { fields: cleanFieldsForApi(fields as any, 'restaurant') });
  }
  static async updateRestaurantEntry(id: string, fields: Partial<CreateRestaurant>) {
    return callApi('PATCH', `/apps/${APP_IDS.RESTAURANT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'restaurant') });
  }
  static async deleteRestaurantEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.RESTAURANT}/records/${id}`);
  }
